...
export const InputField: FC<InputFieldProps> = ({register, field, watchValue, errors}) => {
  return <Box w={'100%'}>
    <Flex>
    ...
    <Input
      ...
      color={'ui.20'}
      bg={'ui.5'} placeholder={field.title} {...register(field.title, field.validations)}/>
    <Flex justifyContent={'space-between'}>
      <Text color={'accent.red'} fontSize={'14px'} fontWeight={400}>
        {errors?.message}
      </Text>
      <Box h={'18px'}>
        {field.options?.hasPasswordStrength && <PasswordStrength text={watchValue}/>}
        {field.options?.hasLengthCounter &&
        <Text fontSize={'14px'} fontWeight={400}
              color={field.maxLength && watchValue?.length > field?.maxLength ? 'accent.red' : 'ui.20'}>
