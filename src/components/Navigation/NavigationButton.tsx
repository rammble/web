...
export const NavigationButton: FC<NavigationButtonProps> = ({page}) => {
  ...
  return (
      <HStack w={'200px'} h={'48px'} bg={isActive ? 'ui.3' : 'bg'} p={2} cursor={'pointer'} borderRadius={8} gap={4} onClick={() => router.push(page.path as string)}>
        <Icon color={isActive ? 'ui.90' : 'ui.60'} boxSize={6}>{page.icon}</Icon>
